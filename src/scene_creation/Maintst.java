...
public class Maintst extends JPanel implements KeyListener, MouseListener, MouseMotionListener {
	...
	private static Alpha[] alpha = new Alpha[3];
	...
	private TransformGroup zoomedBookTG;
	private Transform3D originalBookTransform = new Transform3D();
	...
	public static BranchGroup create_Scene() {
		...
		sceneBG.addChild(CommonsNG.add_Lights(CommonsNG.White, 1));
		return sceneBG;
	}
	...
	private void playYouLoseSound() {
		try {
			URL soundURL = Maintst.class.getResource("youlose.wav");
			if (soundURL == null) {
			...
			while (node != null) {
				...
				if (node instanceof TransformGroup) {
					...
					if ("book".equals(userData)) {
						// Store the TransformGroup and its original transform
	                    zoomedBookTG = tg;
	                    tg.getTransform(originalBookTransform);
	                 
	                    // Apply the zoom effect (since zoomObject is not defined, implement it inline)
	                    Transform3D scaleTransform = new Transform3D();
	                    scaleTransform.setScale(1.1);
	                    Transform3D newTransform = new Transform3D(originalBookTransform);
	                    newTransform.mul(scaleTransform);
	...
	public void resetBookZoom() {
	    if (zoomedBookTG != null) {
	        zoomedBookTG.setTransform(originalBookTransform);
	        zoomedBookTG = null; // Clear the reference after resetting
	    }
	}
	...
	private static void startBackgroundSound() {
		try {
			URL soundURL = Maintst.class.getResource("Horrorsound.wav");
			if (soundURL == null) {
